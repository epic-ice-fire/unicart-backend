...
class _LobbyScreenState extends State<LobbyScreen> {
  ...
  // Tracks the Flutterwave URL per item so user can reopen without re-initialising
  ...
  /// Opens a Flutterwave payment URL across all browsers.
  ///
  /// On web (Chrome, Safari, Brave — desktop & mobile):
  ///   Uses html.window.open() directly with "_blank". This is called from
  ///   a user-gesture chain (button tap → async API call → here), which all
  ///   major browsers accept. Safari on iOS may block the popup; we fall back
  ///   to same-tab navigation via window.location.href in that case.
  ///
  /// On native: uses url_launcher with externalApplication mode.
  Future<bool> _openPaymentUrl(String url) async {
    if (kIsWeb) {
      try {
        final win = html.window.open(url, "_blank");
        if (win == null) {
          // Popup was blocked — navigate same tab instead
  ...
  /// Initialise entry fee payment and open Flutterwave.
  /// Uses _openPaymentUrl() which correctly handles Safari / Chrome / Brave
  /// on both desktop and mobile web, plus native via url_launcher.
  Future<void> startEntryFeePayment() async {
    ...
    try {
      ...
      setState(() {
        ...
        pendingPaymentUrl = authUrl;
      });
      if (authUrl != null && authUrl.isNotEmpty) {
        _startPolling();
        final launched = await _openPaymentUrl(authUrl);
        showMessage(launched
            ? "Flutterwave opened. Pay then tap \"I've Paid\" when you return."
            : "Could not open payment page. Tap \"Open Flutterwave\" below to try again.");
      }
    } catch (e) {
  ...
  Future<void> reopenPaymentLink() async {
    ...
    final launched = await _openPaymentUrl(pendingPaymentUrl!);
    if (!launched) showMessage("Could not open payment page. Please try again.");
  }
    ...
    try {
      ...
      if (authUrl != null && authUrl.isNotEmpty) {
        ...
        showMessage(itemLaunched
            ...
            : "Could not open payment page — tap \"Open Flutterwave\" on the item below.");
      }
      await loadAll();
    } catch (e) { showMessage(e.toString()); }
    finally { if (mounted) setState(() => isBusy = false); }
                                    ...
                                    OutlinedButton.icon(
                                      onPressed: isBusy ? null : () async {
                                        ...
                                        if (url != null && url.isNotEmpty) {
                                          ...
                                          // Re-initialise if URL was lost (e.g. page refresh)
